...
const BookingPage = () => {
  ...
  const [bookingType, setBookingType] = useState(location.state?.type || 'reguler'); // 'reguler' | 'membership'
  ...
  // State for 4 membership sessions
  ...
  const [takenSlots, setTakenSlots] = useState({}); // { 'YYYY-MM-DD': [{start, duration, court}, ...] }

  useEffect(() => {
    // Set default date to tomorrow for regular booking
    ...
    // Set default dates for membership (week 1, 2, 3, 4 from tomorrow)
    const newSessions = [...sessions];
    for(let i = 0; i < 4; i++) {
      const d = new Date(tomorrow);
      d.setDate(d.getDate() + (i * 7)); // every week
      newSessions[i].date = d.toISOString().split('T')[0];
    }
  ...
  // Fetch taken slots when dates change
  useEffect(() => {
  ...
  const getTakenSlot = (date, time, duration, courtName) => {
    const daySlots = takenSlots[date] || [];
    if (daySlots.length === 0) return null;

    const start1 = new Date(`1970-01-01T${time}`);
    const end1 = new Date(start1.getTime() + duration * 60 * 60 * 1000);

    return daySlots.find(slot => {
      if (slot.court_name !== courtName) return false;
      const start2 = new Date(`1970-01-01T${slot.start_time}`);
      const end2 = new Date(start2.getTime() + slot.duration * 60 * 60 * 1000);
  ...
  const totalPrice = bookingType === 'reguler'
    ...
    : (formData.court.price * formData.duration * 4) * 0.75; // 25% discount for 4 sessions
  ...
  const handleSubmit = async (e) => {
    ...
    // Validate dates and times before submitting
    if (bookingType === 'reguler') {
      if (formData.date < today) {
        setErrorModal({ show: true, message: `Tanggal ${formData.date} sudah terlewati. Silakan pilih tanggal hari ini atau yang akan datang.` });
        setLoading(false);
        return;
      }
      if (formData.date === today) {
        ...
        if (selectedHour <= now.getHours()) {
          setErrorModal({ show: true, message: `Jam ${formData.time} untuk hari ini sudah terlewati. Silakan pilih jam yang belum lewat.` });
          setLoading(false);
          return;
        }
      }
      ...
      for (const session of sessions) {
        if (session.date < today) {
          setErrorModal({ show: true, message: `Sesi tanggal ${session.date} sudah terlewati. Silakan ganti ke tanggal yang akan datang.` });
          setLoading(false);
          return;
        }
        if (session.date === today) {
          ...
          if (selectedHour <= now.getHours()) {
            setErrorModal({ show: true, message: `Sesi jam ${session.time} pada hari ini sudah terlewati. Silakan pilih jam yang belum lewat.` });
            setLoading(false);
            return;
          }
        }
    ...
    try {
      ...
      if (bookingType === 'reguler') {
        ...
        payload.membership_sessions = sessions;
      }
    ...
    <div className="min-h-screen bg-dark pt-24 pb-12 px-4 sm:px-6 lg:px-8">
                ...
                <div className="grid sm:grid-cols-2 gap-6">
                  <div>
                    ...
                    <input 
                      ...
                      value={formData.date}
                      onChange={(e) => { e.target.setCustomValidity(''); setFormData({ ...formData, date: e.target.value }); }}
                      onInvalid={(e) => e.target.setCustomValidity('Waktu yang kamu pilih sudah terlewat. Pilih tanggal hari ini atau yang akan datang.')}
                  ...
                  <div>
                    ...
                    <input 
                      ...
                      className={`w-full bg-white/5 border rounded-xl px-4 py-3 text-white focus:outline-none transition-all ${
                        getTakenSlot(formData.date, formData.time, formData.duration, formData.court.name)
                    ...
                    {getTakenSlot(formData.date, formData.time, formData.duration, formData.court.name) && (
                      <p className="text-[10px] text-red-500 mt-2 font-bold uppercase tracking-wider flex items-center gap-1">
                        <XCircle size={12} /> 
                        {getTakenSlot(formData.date, formData.time, formData.duration, formData.court.name).status === 'closed' 
                          ? 'Lapangan ditutup / Maintenance' 
                          : 'Jadwal sudah terisi'}
                      </p>
                    )}
                  </div>
                      ...
                      <div className="grid grid-cols-2 gap-4 flex-1">
                        <div>
                          ...
                          <input 
                            ...
                            min={new Date().toISOString().split('T')[0]}
                            value={session.date}
                            onChange={(e) => { e.target.setCustomValidity(''); handleSessionChange(index, 'date', e.target.value); }}
                            onInvalid={(e) => e.target.setCustomValidity('Waktu yang kamu pilih sudah terlewat. Pilih tanggal hari ini atau yang akan datang.')}
                        ...
                        <div>
                          ...
                          <input 
                            ...
                            className={`w-full bg-transparent border-b px-2 py-1 text-white focus:outline-none transition-all ${
                              getTakenSlot(session.date, session.time, formData.duration, formData.court.name)
                                ? 'border-red-500'
                          ...
                          {getTakenSlot(session.date, session.time, formData.duration, formData.court.name) && (
                            <p className="text-[9px] text-red-500 mt-1 font-bold uppercase">
                              {getTakenSlot(session.date, session.time, formData.duration, formData.court.name).status === 'closed' 
                                ? 'CLOSED' 
                                : 'TERISI'}
                            </p>
                          )}
            ...
            <div className="glass p-6 rounded-2xl border border-white/10 sticky top-28">
              ...
              <button
                ...
                className={`w-full py-4 text-white font-bold rounded-xl hover:shadow-lg hover:-translate-y-1 transition-all flex items-center justify-center gap-2 disabled:opacity-50 ${
                  bookingType === 'membership' ? 'bg-gradient-to-r from-secondary to-yellow-500 hover:shadow-secondary/30' : 'bg-gradient-to-r from-primary to-accent hover:shadow-primary/30'
                }`}
              >
      ...
      {errorModal.show && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          <div className="absolute inset-0 bg-black/80 backdrop-blur-md animate-fade-in" onClick={() => setErrorModal({ ...errorModal, show: false })}></div>
